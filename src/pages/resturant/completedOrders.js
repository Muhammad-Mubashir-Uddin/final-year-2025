...
const API_URL = "/api/resturant/completedOrders";
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        setError(err.message || "Error fetching completed orders");
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className="modal-overlay" onClick={() => setSelectedOrder(null)}>
              <div className="modal" onClick={e => e.stopPropagation()}>
                ...
                {selectedOrder.completionDate && (
                  <p><strong>Completed At:</strong> {new Date(selectedOrder.completionDate).toLocaleString()}</p>
                )}
...
const CompletedOrdersStyled = styled.div`
  .container {
    background: #fff;
    border-radius: 15px;
    padding: 20px;
